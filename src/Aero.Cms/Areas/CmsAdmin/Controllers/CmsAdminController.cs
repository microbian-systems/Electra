using Aero.Cms.Areas.CmsAdmin.Models;
...
namespace Aero.Cms.Areas.CmsAdmin.Controllers;
...
public class CmsAdminController : Controller
{
    ...
    private readonly RoleManager<AeroRole> _roleManager;
    ...
    public async Task<IActionResult> AddUser()
    {
        ...
        {
            User = new AeroUser { UserName = "", Email = "" },
    ...
    public async Task<IActionResult> SaveUser(string userId, [FromForm] AeroUser model, [FromForm] string? password, [FromForm] string[] selectedRoles)
    {
        AeroUser? user;
    ...
    {
        return View("EditRole", new AeroRole());
    }
    ...
    public async Task<IActionResult> SaveRole(string roleId, [FromForm] AeroRole model)
    {
        if (string.IsNullOrEmpty(roleId) || roleId == "new")
        {
            var result = await _roleManager.CreateAsync(new AeroRole { Name = model.Name });
            if (!result.Succeeded)
            {
                foreach (var error in result.Errors)
                {
    ...
    public async Task<IActionResult> SaveMySettings([FromForm] AeroUser model, [FromForm] string? currentPassword, [FromForm] string? newPassword)
    {
        var user = await _userManager.GetUserAsync(User);
        if (user == null) return RedirectToAction("Login");
