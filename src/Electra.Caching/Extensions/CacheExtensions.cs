...
namespace Electra.Common.Caching.Extensions
{
    public static class CacheClientExtensions
    {
        ...
        public static async Task<int> AddAsync<T>(this ICacheClient cache, string key, IDictionary<string, T> value, CacheOptions opts)
        {
        ...
        public static int AddItems<T>(this ICacheClient cache,
            IEnumerable<(string key, T value, TimeSpan? expiration)> items) => AddItemsAsync(cache, items).GetAwaiter().GetResult();
        ...
        /// <returns></returns>
        public static async Task<int> AddItemsAsync<T>(this ICacheClient cache, IEnumerable<(string key, T value, TimeSpan? expiration)> items)
        {
        ...
        public static bool AddDictionary<T>(this ICacheClient cache, string key, IDictionary<string, T> dict, CacheOptions opts)
        {
        ...
        public static async Task<bool> AddDictionaryAsync<T>(this ICacheClient cache, string key, IDictionary<string, T> dict, CacheOptions opts)
        {
            var ret = await cache.AddAsync(key, dict, opts);
            return true;
        }

        public static async Task<bool> AddDictionaryAsync<T>(this ICacheClient cache, CacheEntry<IDictionary<string, T>> entry)
        ...
        public static async Task<int> AddCollectionAsync<T>(this ICacheClient cache, string key, IEnumerable<T> items, CacheOptions opts)
        {
        ...
        public static async Task<T> GetOrCreateAsync<T>(this ICacheClient cache, string key, Func<T> create, CacheOptions opts = null)
        {
