...
namespace Microbians.Services.Geo
{
    ...
    public class ZipApiService : IZipApiService
    {
        ...
        public async Task<(bool status, (string distance, string unit) data)> GetDistance(string zip1, string zip2, string unit = "mi")
        {
            log.LogInformation($"getting distance bewteen {zip1} and {zip2}");
            
            var json = await client.GetStringAsync($"zipcode/distance/?{param}{apiKey}&zip1={zip1}&zip2={zip2}&unit={unit}");
            var res = JsonSerializer.Deserialize<dynamic>(json);
    ...
    { 
        ...
        [JsonPropertyName("state_fullname")]
