...
{
    public Task<List<T>> GetPaged(int page = 1, int rows = 10);
    public Task SaveChangesAsync();
    public void SaveChanges();
}

public interface IGenericEntityFrameworkRepository<T> : IGenericEntityFrameworkRepository<T, Guid>,
    IGenericRepository<T> where T : class, IEntity<Guid>, new()
{
}

public class GenericEntityFrameworkRepository<T> : GenericEntityFrameworkRepository<T, Guid>, IGenericRepository<T>
    where T : class, IEntity<Guid>, new()
{
    public GenericEntityFrameworkRepository(DbContext context, ILogger<GenericEntityFrameworkRepository<T>> log) :
        base(context, log)
    {
    }
}
...
public class GenericEntityFrameworkRepository<T, TKey> : GenericRepository<T, TKey>,
    IGenericEntityFrameworkRepository<T, TKey> where T : class, IEntity<TKey>, new() where TKey : IEquatable<TKey>
{
    protected readonly DbSet<T> db;
    protected readonly DbContext context;

    protected GenericEntityFrameworkRepository(DbContext context,
    ...
    where TContext : DbContext
{
    ...
    protected GenericEntityFrameworkRepository(TContext context,
        ILogger<GenericEntityFrameworkRepository<T, TKey, TContext>> log) : base(log)
    {
        this.db = context.Set<T>();
        this.context = context;
    }
