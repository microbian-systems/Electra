...
{
    /// <summary>
    /// Gets media items from a picker using the user's profile
    /// </summary>
    /// <param name="profile">The UserProfile to get the media item from</param>
    ...
    public static async Task<IEnumerable<Media.Models.Media?>> GetMedia(this CmsUserProfile profile, string alias, IMediaService mediaService, string? fallBackUrl = null)
    {
        var mediaIds = profile.GetValue<List<string>?>(alias);
        if (mediaIds != null)
        {
    ...
    /// <summary>
    /// Gets content items from a picker using the user's profile
    /// </summary>
    /// <param name="profile">The UserProfile to get the media item from</param>
    /// <param name="alias">The property alias</param>
    /// <param name="contentService">An injected contentService</param>
    /// <returns>List of Content</returns>
    public static async Task<IEnumerable<Content.Models.Content>> GetContent(this CmsUserProfile profile, string alias, IContentService contentService)
    {
        var contentIds = profile.GetValue<List<string>?>(alias);
        if (contentIds != null)
        {
