    ...
    where TUser : ElectraUser
{
    ...
    public void Migrate()
    {
        ...
        var newUsers = this.Stream<TUser>()
            .Where(u => !userIdsToDelete.Contains(u.Id.ToString())) // Exclude the old users; we're going to delete those momentarily if everything else succeeds.
            .Select(u => (email: u.Email, id: u.Id!))
            .ToList();
        var migratedEmailReservations = 0;
        foreach (var (email, id) in newUsers)
        {
            ...
            UpdateEmailReservation(email, id.ToString());
        }
    ...
    private List<string> CloneUserWithNewId(IEnumerable<StreamResult<TUser>> users)
    {
        ...
        foreach (var userStream in users)
        {
            ...
            if (this.newUserIdType == UserIdType.Consecutive && newId != null)
            {
            ...
            var needsNewId = !string.Equals(newId, user.Id.ToString(), StringComparison.OrdinalIgnoreCase);
            if (needsNewId)
            {
                // Strip off the existing ID, otherwise bulk insert will pick it up and use the existing.
                user.Id = Snowflake.NewId();
                ...
                var isNewId = !string.Equals(user.Id.ToString(), userStream.Id, StringComparison.OrdinalIgnoreCase);
                if (isNewId)
                {
